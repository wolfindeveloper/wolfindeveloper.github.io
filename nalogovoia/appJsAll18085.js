...
(function ($) {
    $.fn.nicefileinput = function (options) {
        var settings = {
            browse: 'ÐžÐ±Ð·Ð¾Ñ€...', // Default button text
...
function isChromiumBased()
{
    ...
    //Ð½ÐµÐºÐ¾Ñ‚Ð¾Ñ€Ñ‹Ñ… Ð²ÐµÑ€ÑÐ¸ÑÑ… IE8 Ñ Ð¿Ð¾Ð´ÐºÐ»ÑŽÑ‡ÐµÐ½Ð½Ñ‹Ð¼ Ð¿Ð»Ð°Ð³Ð¸Ð½Ð¾Ð¼ chromeframe Ð¾Ð½ Ð¾Ð¿Ñ€ÐµÐ´ÐµÐ»ÑÐµÑ‚ÑÑ ÐºÐ°Ðº
    //Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; chromeframe/29.0.1547.67;
    // Ð¸ Ð¼Ð¾Ð¶ÐµÑ‚ Ð¿Ð¾Ð¿Ð°Ð´Ð°Ñ‚ÑŒ Ð² Ð²ÐµÑ‚ÐºÑƒ Chrome
    ...
    if(retVal_chrome == null) // Ð’ Firefox Ð¸ IE Ñ€Ð°Ð±Ð¾Ñ‚Ð°ÐµÐ¼ Ñ‡ÐµÑ€ÐµÐ· NPAPI
        return false;
    else
    {
        // Ð’ Chrome Ð¸ Opera Ñ€Ð°Ð±Ð¾Ñ‚Ð°ÐµÐ¼ Ñ‡ÐµÑ€ÐµÐ· Ð°ÑÐ¸Ð½Ñ…Ñ€Ð¾Ð½Ð½ÑƒÑŽ Ð²ÐµÑ€ÑÐ¸ÑŽ
...
$(document).ready(function () {
    ...
    $('#fancybox').fancybox({
        tpl: {
            ...
            '<div id="player">' + // player container replaces fancybox-inner
            (Ext.isIE8
                ? '<div style="margin: 0 auto; padding: 200px 0 0; text-align:center; width: 820px;height:450px; border: 1px solid #ccc;">Ð’Ð°Ñˆ Ð±Ñ€Ð°ÑƒÐ·ÐµÑ€ Ð½Ðµ ÑÐ¼Ð¾Ð³ Ð¾Ñ‚Ð¾Ð±Ñ€Ð°Ð·Ð¸Ñ‚ÑŒ ÑÑ‚Ð¾ Ð²Ð¸Ð´ÐµÐ¾</div>'
                :
                (window.isMobile.any
                    ? '<div style="margin: 0 auto; padding: 200px 0 0; text-align:center; width: 820px;height:450px; border: 1px solid #ccc;">Ð‘Ñ€Ð°ÑƒÐ·ÐµÑ€ Ð½Ðµ Ð¿Ð¾Ð´Ð´ÐµÑ€Ð¶Ð¸Ð²Ð°ÐµÑ‚ Ð¿Ñ€Ð¾Ð¸Ð³Ñ€Ñ‹Ð²Ð°Ð½Ð¸Ðµ Ð´Ð°Ð½Ð½Ð¾Ð³Ð¾ Ð²Ð¸Ð´ÐµÐ¾.</div>'
                    : checkFlashInBrowser()
                    ?
                ...
                'Ð”Ð»Ñ Ð¿Ñ€Ð¾ÑÐ¼Ð¾Ñ‚Ñ€Ð° Ð²Ð¸Ð´ÐµÐ¾ Ð²Ð°Ð¼ Ð½ÐµÐ¾Ð±Ñ…Ð¾Ð´Ð¸Ð¼Ð¾ ÑƒÑÑ‚Ð°Ð½Ð¾Ð²Ð¸Ñ‚ÑŒ <a href="https://get.adobe.com/ru/flashplayer/" target="_blank">Adobe Flash Player</a>.' +
                '</div>')) +
            '</div></div></div></div>'
        }
    });
...
/**
 * ÐžÑ‚ÐºÑ€Ñ‹Ð²Ð°ÐµÐ¼ Ð±Ð»Ð¾Ðº Ñ Ð¾Ñ‚Ð²ÐµÑ‚Ð°Ð¼Ð¸
 *
 * @param id
 */
...
/**
 * ÐÐ°Ð¿Ð¾Ð»Ð½ÑÐµÐ¼ Ð±Ð»Ð¾Ðº Ñ Ñ‚ÐµÐ¼Ð°Ñ‚Ð¸ÐºÐ°Ð¼Ð¸
 *
 * @param el
 * @param id
 */
...
/**
 * Ð£ÑÑ‚Ð°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ ÑÐµÐºÑ†Ð¸Ð¸
 * @param items
 * @param el
 */
...
function toggleAnswer(el) {
    // el - ÑÑÑ‹Ð»ÐºÐ° Ð½Ð° Ð±Ð»Ð¾Ðº answer
...
$(function () {
    if (!areCookiesEnabled()) {
        ...
        $("#form-submit-login").click(function (e) {
            ...
            if (12 !== name.length) {
                showError('ÐÐµÐ¿Ñ€Ð°Ð²Ð¸Ð»ÑŒÐ½Ñ‹Ð¹ Ð»Ð¾Ð³Ð¸Ð½ Ð¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÐµÐ»Ñ.');
                return false;
            } else if (0 === pass.length) {
                showError('ÐÐµÐ¿Ñ€Ð°Ð²Ð¸Ð»ÑŒÐ½Ñ‹Ð¹ Ð¿Ð°Ñ€Ð¾Ð»ÑŒ Ð¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÐµÐ»Ñ.');
                return false;
            }

            if (countTry >=3) {
                if (6 !== captcha.length) {
                    showError('ÐÐµÐ¿Ñ€Ð°Ð²Ð¸Ð»ÑŒÐ½Ð¾ Ð²Ð²ÐµÐ´ÐµÐ½Ñ‹ Ñ†Ð¸Ñ„Ñ€Ñ‹ Ñ ÐºÐ°Ñ€Ñ‚Ð¸Ð½ÐºÐ¸.');
                    return false;
                } else {
            ...
            var defaultError = 'Ð¡ÐµÑ€Ð²Ð¸Ñ Ð²Ñ€ÐµÐ¼ÐµÐ½Ð½Ð¾ Ð½ÐµÐ´Ð¾ÑÑ‚ÑƒÐ¿ÐµÐ½ Ð¿Ð¾ Ñ‚ÐµÑ…Ð½Ð¸Ñ‡ÐµÑÐºÐ¸Ð¼ Ð¿Ñ€Ð¸Ñ‡Ð¸Ð½Ð°Ð¼. ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð¿Ð¾Ð²Ñ‚Ð¾Ñ€Ð¸Ñ‚Ðµ Ð¿Ð¾Ð¿Ñ‹Ñ‚ÐºÑƒ Ð¿Ð¾Ð·Ð¶Ðµ.';
            request.send(
                {
                    ...
                    if (false == resp) {
                        ...
                        if (1 == resp.status
                            ...
                            if (rememberMeCheckbox.attr('checked')) {
                                //ÐµÑÐ»Ð¸ Ð½Ðµ ÑƒÑÑ‚Ð½Ð¾Ð²Ð¸Ñ‚ÑŒ Ð°Ñ‚Ñ€Ð¸Ð±ÑƒÑ‚ expires Ñ‚Ð¾ ÐºÑƒÐºÐ¸ Ð²Ñ‹Ð»ÐµÑ‚ÑÑ‚ Ð¿Ñ€Ð¸ Ð·Ð°ÐºÑ€Ñ‹Ñ‚Ð¸Ð¸ Ð±Ñ€Ð°ÑƒÐ·ÐµÑ€Ð°
                            ...
                            if (typeof resp.response == "undefined"
                                ...
                                if(resp.response.data == '10') {
                                    showError('Ð’Ñ‹ Ð²Ð²ÐµÐ»Ð¸ Ð½ÐµÐ²ÐµÑ€Ð½Ñ‹Ð¹ Ð¿Ð°Ñ€Ð¾Ð»ÑŒ. ÐŸÑ€Ð¾ÑÑŒÐ±Ð° Ð¿Ð¾Ð²Ñ‚Ð¾Ñ€Ð¸Ñ‚ÑŒ Ð¿Ð¾Ð¿Ñ‹Ñ‚ÐºÑƒ.');
                                } else {
    ...
    function showError(text) {
        text = text.length > 0 ? text : 'Ð¡ÐµÑ€Ð²Ð¸Ñ Ð²Ñ€ÐµÐ¼ÐµÐ½Ð½Ð¾ Ð½ÐµÐ´Ð¾ÑÑ‚ÑƒÐ¿ÐµÐ½ Ð¿Ð¾ Ñ‚ÐµÑ…Ð½Ð¸Ñ‡ÐµÑÐºÐ¸Ð¼ Ð¿Ñ€Ð¸Ñ‡Ð¸Ð½Ð°Ð¼.';
    ...
    $("form").ajaxForm(function(responseText, status, responseObject) {
        ...
        if ('success' == status) {
            ...
            if (1 == responseObject.success) {
                ...
                showError(responseObject.response.length > 0 ? responseObject.response : 'ÐŸÑ€Ð¾Ð¸Ð·Ð¾ÑˆÐ»Ð° Ð¾ÑˆÐ¸Ð±ÐºÐ° Ð´Ð¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ñ Ñ„Ð°Ð¹Ð»Ð°. Ð’Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ñ€Ð°Ð·Ð¼ÐµÑ€ Ð²Ð°ÑˆÐµÐ³Ð¾ Ñ„Ð°Ð¹Ð»Ð° Ð¿Ñ€ÐµÐ²Ñ‹ÑˆÐ°ÐµÑ‚ 20ÐœÐ±.');
                $('.nicefileinput .remove-button').trigger('click');
            }
        } else {
            showError('ÐŸÑ€Ð¾Ð¸Ð·Ð¾ÑˆÐ»Ð° Ð¾ÑˆÐ¸Ð±ÐºÐ° Ð´Ð¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ñ Ñ„Ð°Ð¹Ð»Ð°. Ð’Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ñ€Ð°Ð·Ð¼ÐµÑ€ Ð²Ð°ÑˆÐµÐ³Ð¾ Ñ„Ð°Ð¹Ð»Ð° Ð¿Ñ€ÐµÐ²Ñ‹ÑˆÐ°ÐµÑ‚ 20ÐœÐ±.');
            $('.nicefileinput .remove-button').trigger('click');
        }
    });
    $('#newCaptcha').click(function() {
    ...
    $('#form-submit').on('click', function(e) {
        ...
        $.each(formValues, function(index, item) {
            ...
            if (!item.value && $.inArray(item.name, notRequired) == '-1') {
                error.push('Ð’Ñ‹ Ð·Ð°Ð¿Ð¾Ð»Ð½Ð¸Ð»Ð¸ Ð½Ðµ Ð²ÑÐµ Ð¾Ð±ÑÐ·Ð°Ñ‚ÐµÐ»ÑŒÐ½Ñ‹Ðµ Ð¿Ð¾Ð»Ñ');
                return false;
            }
            ...
            switch(item.name) {
                case 'inn':
                    if (item.value.length != 12) {
                        error.push('ÐŸÐ¾Ð»Ðµ "Ð˜ÐÐ" Ð´Ð¾Ð»Ð¶Ð½Ð¾ ÑÐ¾Ð´ÐµÑ€Ð¶Ð°Ñ‚ÑŒ 12 ÑÐ¸Ð¼Ð²Ð¾Ð»Ð¾Ð².');
                    }
                    break;
                case 'ogrnIp':
                    if (item.value.length != 15) {
                        error.push('ÐŸÐ¾Ð»Ðµ "ÐžÐ“Ð ÐÐ˜ÐŸ" Ð´Ð¾Ð»Ð¶Ð½Ð¾ ÑÐ¾Ð´ÐµÑ€Ð¶Ð°Ñ‚ÑŒ 15 ÑÐ¸Ð¼Ð²Ð¾Ð»Ð¾Ð².');
                    }
                    if (! isAuth) {
                        ...
                        if (checkNumber != checkWith) {
                            error.push('ÐŸÐ¾Ð»Ðµ "ÐžÐ“Ð ÐÐ˜ÐŸ" Ð²Ð²ÐµÐ´ÐµÐ½Ð¾ Ð½ÐµÐ²ÐµÑ€Ð½Ð¾.');
                        }
                    }
                    break;
                case 'email':
                    if (!isEmail(item.value)) {
                        error.push('ÐŸÐ¾Ð»Ðµ "E-mail" Ð´Ð¾Ð»Ð¶Ð½Ð¾ ÑÐ¾Ð´ÐµÑ€Ð¶Ð°Ñ‚ÑŒ Ð°Ð´Ñ€ÐµÑ ÑÐ»ÐµÐºÑ‚Ñ€Ð¾Ð½Ð½Ð¾Ð¹ Ð¿Ð¾Ñ‡Ñ‚Ñ‹.');
                    }
                    break;
                case 'captcha':
                    if (!isCaptcha(item.value)) {
                        error.push('ÐŸÐ¾Ð»Ðµ "Ð¦Ð¸Ñ„Ñ€Ñ‹ Ñ ÐºÐ°Ñ€Ñ‚Ð¸Ð½ÐºÐ¸" Ð´Ð¾Ð»Ð¶Ð½Ð¾ ÑÐ¾Ð´ÐµÑ€Ð¶Ð°Ñ‚ÑŒ 6 Ñ†Ð¸Ñ„Ñ€.');
                    }
                    break;
            }
        });
        ...
        request.send(
            ...
            function(resp) {
                ...
                if (false !== resp) {
                    ...
                    showError('ÐŸÑ€Ð¾Ð¸Ð·Ð¾ÑˆÐ»Ð° Ñ‚ÐµÑ…Ð½Ð¸Ñ‡ÐµÑÐºÐ°Ñ Ð¾ÑˆÐ¸Ð±ÐºÐ°.')
                }
            }
        );
...
$.fn.select=function(o){
        ...
        $.fn.select.method={
            "destroy":function(select){
                if(select.data('customized')){
                    ...
                    throw new Error('Ð¾Ð±ÑŠÐµÐºÑ‚ Ð½Ðµ Ð¿Ñ€Ð¾Ð¸Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð¸Ñ€Ð¾Ð²Ð°Ð½');
                }
            },
    ...
    if(select.length){
        ...
        throw Error('ÑÐµÐ»ÐµÐºÑ‚/Ñ‹ ÑƒÐ¶Ðµ Ð¿Ñ€Ð¾Ð¸Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð¸Ñ€Ð¾Ð²Ð°Ð½/Ñ‹');
    }
}